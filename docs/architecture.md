...
- **取向：两者都要，但边界划清**——单次调用仍是一个事务（要么全提交要么全回滚，不引入"半整合"），
  但单次只处理预算内的记忆（`DEFAULT_CONSOLIDATION_BUDGET = 2000`，装配注入），超出部分留给下一次；
...
- 返回值分层：报告 = "改了什么"（四个计数，形状是稳定契约）；`processed` / `budget_exhausted` /
  `next_scope_offset` / `dirty` 经 `ConsolidationOutcome` 单独给出（"跑了多少"）。
