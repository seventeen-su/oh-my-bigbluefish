/**
 * `@omb/memory-vector` 组件包入口 —— `cordis.patch.yml` 的 `omb-memory-vector` 行加载的就是它。
 *
 ...
 *
 * 真源仍在 `modules/memory/vector.ts`：本文件只做两件转发，**不复制任何实现**——
 * - 具名导出（含真正的 `vectorModule`）供内核原生装配路径使用；
 * - `default = toHostPlugin(registration)` 供宿主逐行加载使用（它读的就是 default）。
 */
import * as component from '../../modules/memory/vector.js'
import { VECTOR_MODULE_ID } from '../../modules/memory/vector.js'
import { registrationFor } from '../../dsh/modules.js'
import { toHostPlugin } from '../../kernel/hostEntry.js'

export * from '../../modules/memory/vector.js'

export default toHostPlugin(registrationFor(component, VECTOR_MODULE_ID))
